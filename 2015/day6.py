...
def getCoords(line: str)-> tuple:
   ...
   cleanUp = lambda x: [int(coord.strip()) for coord in x.split(',')]
...
def append(lighths: list, x: int, y: int, value: int) -> list:
   lighths[x][y] += value
   if lighths[x][y] < 0: lighths[x][y] = 0
   return lighths
...
def lightCount1(programInput: list) -> int:
   return lightCount(programInput, 
                     { TURN_ON :lambda x,y, lighths: assign(lighths, x, y, 1),
                       TURN_OFF : lambda x,y, lighths: assign(lighths, x, y, 0),
                       TOGGLE : lambda x,y, lighths: assign(lighths, x, y, 1 if lighths[x][y] == 0 else 0)})
