...
def fillEggnog(containers: list[int], remainingCapacity: int, current: list[int] = None, capacities: dict[int, int] = None) -> int:
    ...
    return sum(fillEggnog(temp := temp[1:], remainingCapacity - x, current + [x], capacities) for x in containers)
