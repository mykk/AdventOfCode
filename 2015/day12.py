...
def addParsedNumbers(programInput, ignoreDict: callable) -> int:
    ...
    if isinstance(programInput, dict):
        if ignoreDict(programInput): return 0
        return sum([addParsedNumbers(x, ignoreDict) for x in programInput.values()]) + sum([addParsedNumbers(x, ignoreDict) for x in programInput])
...
def main():
   ...
   print("part 2: ", addNumbers(input_str, lambda x: 'red' in x.keys() or 'red' in x.values()))
