...
def appendDictIfNotFound(
    destinations: dict[str, dict[str, int]], location: str, destination: str, price: int
):
...
def findBestPath(
    ...
    for startPoint in destinations:
        ...
        while toVisit:
            ...
            if not continueSearch(cost, bestCost):
                continue
            ...
            for dest in [x for x in destinations[location] if x not in visited]:
                toVisit.append(
                    (dest, visited | {dest}, cost + destinations[location][dest])
                )
    return bestCost
...
def findShortestPath(destinations: dict[str, dict[str, int]]) -> int:
    return findBestPath(
        destinations, lambda x, y: x < y or y == 0, lambda x, y: x < y or y == 0
    )
