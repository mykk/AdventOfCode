...
def lhsCombinationBetter(lhs: Combination, rhs: Combination, target_load: int) -> bool:
    ...
    if sum(rhs.container) != target_load:
        if lhs_len <= rhs_len: return True
...
def balanceLoad(items: list[int], containers: int, optimizeForFront: bool) -> Combination:
    if containers == 1:
        return Combination(container=items.copy(), remaining_items=[])
    target_load: int = sum(items) // containers
    best_combo: Combination = None
    combinations = [Combination(container=[], remaining_items=sorted(items, key=lambda x: -x))]
    ...
    while combinations:
        ...
        for item in combination.remaining_items:
            ...
            new_combo = Combination(container=combination.container + [item], remaining_items=new_remainder.copy())
            if lhsCombinationBetter(best_combo, new_combo, target_load):
                continue
            
            if new_load == target_load:
                combination.remaining_items.remove(item)
                if balanceLoad(combination.remaining_items, containers - 1, False) is not None:
                    if not optimizeForFront: return new_combo
...
def main():
    ...
    print("part 1: ", prod(balanceLoad([int(x) for x in programInput], 3, True).container))
    print("part 2: ", prod(balanceLoad([int(x) for x in programInput], 4, True).container))
