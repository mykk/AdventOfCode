...
INSTRUCTION_MAP: dict[str, Callable[[any], int]] = {
    "hlf" : lambda r, reg_entry, pos: (reg_entry.update({r :  reg_entry.setdefault(r, 0) / 2}), pos + 1)[1],
    "tpl" : lambda r, reg_entry, pos: (reg_entry.update({r : reg_entry.setdefault(r, 0) * 3}), pos + 1)[1],
    "inc" : lambda r, reg_entry, pos: (reg_entry.update({r : reg_entry.setdefault(r, 0) + 1}), pos + 1)[1],
    "jmp" : lambda offset, reg_entry, pos: pos + offset,
    "jie" : lambda r, offset, reg_entry, pos: pos + offset if reg_entry.setdefault(r, 0) % 2 == 0 else pos + 1,
    "jio" : lambda r, offset, reg_entry, pos: pos + offset if reg_entry.setdefault(r, 0) == 1 else pos + 1
    }
