...
def parseItems(items: str) -> dict[str, dict[str, int]]:
    return {
        name: {"cost": int(cost), "dmg": int(dmg), "armor": int(armor)}
        for name, cost, dmg, armor in (item.split() for item in items)
    }
...
def fightBoss(hero: dict[str, int], boss: dict[str, dict]) -> bool:
    ...
    return math.ceil(hero["health"] / dmgToHero) >= math.ceil(
...
def shopAndFight(
    ...
    if "weapons" in shop:
        ...
        for weapon in shop["weapons"]:
            cost = buyItemAndFight(
                {"armor": shop["armor"], "rings": shop["rings"]},
                hero,
                boss,
                shop["weapons"][weapon],
                forwardParams,
            )
            if comparePriceCallback(cost, bestCost):
                bestCost = cost
        return bestCost
    elif "armor" in shop:
        bestCost = initValue
    ...
    for ring1 in list(shop["rings"]) + [None]:
        for ring2 in list(shop["rings"]) + [None]:
            if ring1 == ring2 and ring1 is not None:
                continue
            tempHero = hero.copy()
            cost = appendItem(tempHero, shop["rings"].get(ring1, None)) + appendItem(
                tempHero, shop["rings"].get(ring2, None)
            )
            if comparePriceCallback(cost, bestCost) and fightBossCallback(
                tempHero, boss
            ):
                bestCost = cost
    return bestCost
...
def main(weapons, armor, rings):
    ...
    bossArmor = [
        int(x.removeprefix("Armor: ")) for x in programInput if x.startswith("Armor")
    ][0]

    cheapestWin = shopAndFight(
