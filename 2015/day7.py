...
def parseLHS(values: dict[str, int], lhs: str) -> int:
   parseValue = lambda x : int(x) if x not in values else values[x]
   ops = [x.strip() for x in lhs.split() if x.strip()]
   if len(ops) == 1:
      return parseValue(ops[0])
   if len(ops) == 2:
      assert(ops[0] == 'NOT')
      return ~parseValue(ops[1])
   return OPERATION_MAP[ops[1]](parseValue(ops[0]), parseValue(ops[2]))
   ...
   for key in circuitDict:
      if key.isnumeric():
         for current in circuitDict[key]:
            ...
            if len(current) == 1: initial.append(current[0])
   return initial

def getNewlyAvailableBooklets(key:str, circuitDict: dict[str, list[list[str]]], values:dict[str, int]) -> list[str]:
   if key not in circuitDict: return []
   new = []
   for current in circuitDict[key]:
      if len(current) == 1:
