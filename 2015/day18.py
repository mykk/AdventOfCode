...
def parseInput(programInput: list[str]) -> dict[complex, bool]:
    return { complex(i, j) : c == '#' for i, row in enumerate(programInput) for j, c in enumerate(row) }

def getNewState(cell: complex, grid: dict[complex, bool], onOffRule: callable) -> bool:
    return onOffRule(grid, cell, sum([1 for i in range(-1, 2) for j in range(-1, 2) if grid.get(cell + complex(i, j), False) and (j != 0 or i != 0)]))

def getNewGrid(grid: dict[complex, bool], count: int, onOffRule: callable) -> dict[complex, bool]:
    if count <= 0:
        return grid
    return getNewGrid({cell : getNewState(cell, grid, onOffRule) for cell in grid}, count - 1, onOffRule)

def getNewGridPart1(grid: dict[complex, bool], count: int) -> dict[complex, bool]:
    onOffRule = lambda grid, cell, onCount: (grid[cell] and (onCount == 2 or onCount == 3)) or (not grid[cell] and onCount == 3)
    return getNewGrid(grid, count, onOffRule)
...
def getNewGridPart2(grid: dict[complex, bool], count: int) -> dict[complex, bool]:
    ...
    onOffRule = lambda grid, cell, onCount: (grid[cell] and (onCount == 2 or onCount == 3)) or (not grid[cell] and onCount == 3) or (cell in corners)
    return getNewGrid({x : x in corners or grid[x] for x in grid}, count, onOffRule)
